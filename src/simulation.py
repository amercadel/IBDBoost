...
def main():
    ...
    if ts_file_name[len(ts_file_name) - 6:] != '.trees.':
        ts_file_name += '.trees'
    afr_population_size = sys.argv[2]
    eur_population_size = sys.argv[3]
    maf_cutoff = float(sys.argv[4])
    genotyping_error_rate = float(sys.argv[5])
    random_seed = int(sys.argv[6])
    sim = stdpopsim(ts_file_name, afr_pop=afr_population_size, eur_pop=eur_population_size, seed = random_seed)
